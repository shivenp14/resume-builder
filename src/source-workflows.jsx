 ...
 */
import { API_BASE, request as sourceRequest } from './api.js';
export { API_BASE, sourceRequest };
...
function Button({ children, kind = 'primary', type = 'button', disabled = false, onClick }) {
  return <button className={`button ${kind}`} type={type} disabled={disabled} onClick={onClick} aria-busy={Boolean(disabled && typeof children === 'string' && children.endsWith('…'))}>{children}</button>;
}
...
function BulletForm({ bullet, onSaved, onCancel, create = false, itemId }) {
  const [form, setForm] = useState({ text: '', tags: '', supporting_facts: '', is_locked: false, is_preferred: false, ...(bullet || {}), tags: textValue(bullet?.tags), supporting_facts: (bullet?.supporting_facts || []).join('\n') });
  const [busy, setBusy] = useState(false); const [error, setError] = useState('');
